...
class PaymentView(APIView):
    ...
    def get(self, request, order_id):
        ...
        alipay_client = AliPay(
            ...
            sign_type="RSA2",
            # 默认False
            debug=settings.dev.ALIPAY_DEBUG
        )
        ...
        order_string = alipay_client.api_alipay_trade_page_pay(
            ...
            return_url="http://www.meiduo.com:8080/pay_success.html",
        )

        # 需要跳转到https://openapi.alipay.com/gateway.do? + order_string
        # 拼接链接返回前端
        alipay_url = settings.dev.ALIPAY_GATEWAY_URL + '?' + order_string
...
class PaymentStatusView(APIView):
    ...
    def put(self, request):
        ...
        # 校验请求参数是否是支付宝的
        alipay_client = AliPay(
            appid=settings.dev.ALIPAY_APPID,
            # 默认回调url
            app_notify_url=None,
            app_private_key_path=os.path.join(os.path.dirname(os.path.abspath(__file__)), "keys/app_private_key.pem"),
            # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥
